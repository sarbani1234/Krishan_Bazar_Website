...
if(isset($_POST['login']))
  {
   $emailcon=$_POST['emailcont'];
    ...
    $query=mysqli_query($con,"select ID from tblcustomer where  (Email='$emailcon' || MobileNumber='$emailcon') && Password='$password' ");
    $ret=mysqli_fetch_array($query);
    if($ret>0){
      $_SESSION['agroctid']=$ret['ID'];
     header('location:dashboard.php');
    }
    else{
    $msg="Your email address or password is incorrect. Please try again. If you've forgotten your sign in details, just click the 'Forgot your password?' link below.";
    }
  }
  ?>
<!DOCTYPE html>
<html lang="zxx"> 
<head>
    <title>KRISHAN BAZAR WEBSITE-Customer's Login Page</title>
  ...
  <div>
    ...
    <div class="buttons">
    <button class="pulse" type="submit" name="login">sign in</button>
  </div>
  <div>
    <a class="forgtpas" href="forgot-password.php">Forgot your password?</a></div>
    <div class="registerit">
      <span style="position: absolute;top: 50px;left: 7%;font-weight: bolder;">NEW CUSTOMER?</span>
    ...
    <a class="accountcr" href="customer_register.php">Create account</a>
    </div>
  </form>
 </div>
 <!--footer starts here-->

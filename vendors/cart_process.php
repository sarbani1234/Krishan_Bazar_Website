...
if(isset($_SESSION["agrovid"]))
{
$duo=$_SESSION["agrovid"];
$ret=mysqli_query($con,"select FullName from tblvendor where ID=$duo");
...
$sql0="SELECT cart_id from `vendor_cart`";
    $runquery=mysqli_query($con,$sql0);
    if (mysqli_num_rows($runquery) == 0) {
        echo( mysqli_error($con));
        $cart_id=1;
    }else if (mysqli_num_rows($runquery) > 0) {
        $sql2="SELECT MAX(cart_id) AS max_val from `vendor_cart`";
...
$sql=mysqli_query($con,"insert into vendor_cart(cart_id,VendorName,Product_Id,Product_name,Product_quantity,Category,Quantity,Unit_price,Total_price) values('$cart_id ','$name','$cpid','$pro_name','$pro_qnty','$cat_name','$qnty','$unit_price','$total_price')") ; 
...
echo '<script>alert("Hope you are enjoying! Make sure that KRISHAN BAZAAR never asks for any personal details. Do not click on unknown links received on mobile/email from any fake sites.")</script>';  
 echo "<script>window.location.href='vendor_billing.php'</script>";

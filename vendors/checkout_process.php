...
if (isset($_SESSION["agrovid"])) {
    ...
    $sql0="SELECT order_id from `vendor_bill`";
    $runquery=mysqli_query($con,$sql0);
    if (mysqli_num_rows($runquery) == 0) {
        echo( mysqli_error($con));
        $order_id=1;
    }else if (mysqli_num_rows($runquery) > 0) {
        $sql2="SELECT MAX(order_id) AS max_val from `vendor_bill`";
	...
	$sql =mysqli_query($con, "INSERT INTO `vendor_bill` (`Invoice_No`,`order_id`,`vendor_id`,`ProductId`,`Quantity`,`name`,`email`,`mobile_no`,`address`,`city`, `state`, `zip`, `cardname`,`cardnumber`,`expdate`,`cvv`) VALUES ('".$invoiceno."',$order_id, '$vendor_id','$cpid','$qnty','$name','$email','$mobilenumber','$address','$city', '$state', '$zip', AES_ENCRYPT('$cardname','".SALT."'),AES_ENCRYPT('$cardnumberstr','".SALT."'),AES_ENCRYPT('$expdate','".SALT."'),AES_ENCRYPT('$cvv','".SALT."'))");
     if($sql) {

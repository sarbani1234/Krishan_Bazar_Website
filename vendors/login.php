...
if(isset($_POST['login']))
  {
   $emailcon=$_POST['emailcont'];
    ...
    $query=mysqli_query($con,"select ID from tblvendor where  (Email='$emailcon' || MobileNumber='$emailcon') && Password='$password' ");
    $ret=mysqli_fetch_array($query);
    if($ret>0){
      $_SESSION['agrovid']=$ret['ID'];
...
<head>
    ...
    <title>KRISHAN BAZAR WEBSITE-Vendor's Login Page</title>
  ...
  <div style="position: absolute;top: 200px;width: 100%;">
    ...
    <input style="position: absolute;top: 180px;left: 50px;border: 2px solid silver;border-radius: 5px;font-family: cursive;color: grey;font-size: 20px;box-sizing: border-box;width: 40%;height: 50px;" type="password" name="password" required="true" placeholder="Password">
                             ...
                             <div class="buttons">
    <button class="close" type="submit" name="login">sign in</button>
  </div>
  <div>
    <a class="forgtpas" href="forgot-password.php">Forgot your password?</a></div>
    <div class="registerit">
      <span style="position: absolute;top: 50px;left: 7%;font-weight: bolder;">NEW VENDOR?</span>
      ...
      <a class="accountcr" href="vendor_register.php">Create account</a>
                                </div>
  </form>
 </div>
 <!--footer starts here-->
